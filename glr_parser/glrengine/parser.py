...
class Parser(object):
    ...
    def conflicts(self):
        ...
        return [i_t for i_t in ((i, t) for i, row in enumerate(self.ACTION)
                       for t in row.keys()) if len(self.ACTION[i_t[0]][i_t[1]]) > 1]

    def count_conflicts(self):
        "Returns the count of conflicts in the ACTION table."
        return reduce(lambda a, b: a + (len(b) > 1 and 1 or 0),
                      (a for row in self.ACTION for a in row.values()),
                      0)
    ...
    def precompute_next_items(self):
        self.next_list = dict((k, set()) for k in self.R if type(k) is str or type(k) is str)
        for item in self.I:
            r, i = item
            n, e, c = self.R[r]
    ...
    def action_to_str(self):
        ...
        def col_width(kw):
            return reduce(max, chain([(type(kw) is str or type(kw) is str) and len(kw) or kw],
                                     (len(cell(i, kw))
